...
// Scrape every product category link from the Drive Medical "PRODUCTS"
// mega-menu and write them (with their group + name) to a CSV.
//
// The menu HTML is grouped into sections; this generalizes the
// browser-copied selector
...
const scrapeDriveMedicalCategories = async () => {
  const res = await fetch(`${BASE}/us/en`, {
  ...
  const csvData = parser.parse(categories)

  await writeFile('Drive Medical categories.csv', '﻿' + csvData)
...
// Lighter version of scrapeDriveMedicalCategories above — returns the category
// list in memory instead of writing it to a CSV, so buildSkuToCategoryMap can
// walk each category.
...
//   .details > a > p   → the product name
//   .details > div a   → one or more <a> variant SKU links
// All variants under a tile share the same product name.
const scrapeCategorySkus = async category => {
  ...
  for (let page = 0; ; page++) {
    ...
    // page, so retry a few times before treating empty as the end.
    let tiles = 0
    let pageProducts = []
    ...
    if (tiles < PAGE_SIZE) break
  }
  return products
}

// Crawl every category and build a sku → { category, name } lookup so each
// row in the product CSV can show which category the SKU belongs to and the
// product's display name. A SKU listed under more than one category gets
// tagged with the first one we encounter.
const buildSkuInfoMap = async () => {
  const categories = await getCategoryLinks()
  console.info(
    `Found ${categories.length} categories. Building SKU info map...`
  )
  const map = new Map()
  await eachOfLimit(categories, 3, async (category, i) => {
    try {
      ...
      for (const { sku, name } of products) {
        if (!map.has(sku)) map.set(sku, { category: category.name, name })
      }
      console.info(
  ...
  console.info(`Mapped ${map.size} unique SKUs.`)
  return map
}
...
const main = async () => {
  ...
  const cookie =
    'us-cartCount=0; ROUTE=.accstorefront-55bf6bf78-r64rv; JSESSIONID=FC63FBE5CA5C4D39290DC613DF35F185.accstorefront-55bf6bf78-r64rv; acceleratorSecureGUID=a47762a2256428fae491b218864cf43fff905bfe; us-userLoggedIn=true'

  // Build the sku → { category, name } lookup before fetching product data so
  // every finalData row can be tagged with its category and product name. SKUs
  // not present in any category listing (e.g. discontinued or B2B-only) fall
  // back to empty strings for both fields.
  // const skuInfo = await buildSkuInfoMap()

  await eachOfLimit(skus, 3, async (sku, i) => {
  ...
  await writeFile(`Drive Medical updated1.csv`, '\ufeff' + csvData)

  process.exit()
}

// Run the category scraper. Swap to `main()` for the SKU price/stock job.
// scrapeDriveMedicalCategories()
main()
